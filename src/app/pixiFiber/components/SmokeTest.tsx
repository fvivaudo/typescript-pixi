...
import { createStageClass, Stage } from "react-pixi-fiber/index.js";
    ...
    <Fragment>
      ...
      {mount && (
        <StageComponent key={useStageClass ? "class" : "function"} options={{ backgroundColor, height, width }}>
          {count > 0 && <RotatingBunny x={width / 2} y={height / 2} textureIndex={0} step={0.1} />}
          {count > 1 && <RotatingBunny x={width / 4} y={height / 4} textureIndex={1} step={0.2} />}
          {count > 2 && <RotatingBunny x={width / 4} y={(3 * height) / 4} textureIndex={2} step={-0.25} />}
          {count > 3 && <RotatingBunny x={(3 * width) / 4} y={height / 4} textureIndex={3} step={-0.1} />}
          {count > 4 && <RotatingBunny x={(3 * width) / 4} y={(3 * height) / 4} textureIndex={4} step={-0.02} />}
        </StageComponent>
      )}
    </Fragment>
  );
